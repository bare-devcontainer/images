...
environment small and auditable: it carries only uv on top of the base, installs software only
from upstreams verified at build time, runs as a non-root user, and is published with SLSA
...
an image pulled from `docker.io/baredevcontainer/uv` verifies with the same command against its
own reference. [Verifying Published Images](../README.md#verifying-published-images) covers
