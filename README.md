...
- **Unprivileged by default** — Each image runs as a non-root user that owns nothing outside its home directory, so no code the container runs can modify the toolchain running it. See [The `dev` user](#the-dev-user).
