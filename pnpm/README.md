...
environment small and auditable: it carries only pnpm on top of the base, installs software
...
| Tags | Debian variant |
|------|----------------|
| `12.3.4-trixie`, `12-trixie`, `12.3.4`, `12`, `trixie` | trixie |
| `12.3.4-bookworm`, `12-bookworm`, `bookworm` | bookworm |

Tags are also published with a date suffix on each build (e.g., `12.3.4-trixie-<YYYYMMDD>`).
<!-- tags:end -->
...
an image pulled from `docker.io/baredevcontainer/pnpm` verifies with the same command against
