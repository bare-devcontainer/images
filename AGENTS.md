...
- The `description` of `build.yaml` is one string with two readers. As the `org.opencontainers.image.description` label and index annotation, GHCR shows it under the package name, above the root README, and `docker inspect` reports it. As the Docker Hub short description, it is all a search result on Docker Hub or in `docker search` shows of the image, with no README or organization name around it to say what the project is. Write it for the second reader: open with `Minimal Dev Container image with <what it ships>`, so the roughly forty characters `docker search` keeps still name the image and its purpose, and close with `Debian, verified upstreams, SLSA provenance`, the traits that set these images apart. Name the software and nothing more; the README carries the details. Docker Hub rejects a description past 100 characters, and `mirror.yml` checks that on every pull request touching `build.yaml`.
