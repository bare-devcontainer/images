...
- No directory `dev` can write is on an image's `PATH`. `ENV PATH` applies to every user in the container, so such a directory would let anything the container runs shadow a command another user resolves, root included. Declare it through `remoteEnv` in the `devcontainer.metadata` label instead, which only a Dev Container client's own processes pick up, with `${containerEnv:PATH}` carrying the image's own `PATH` over. A re-declared label replaces, rather than merges with, the one inherited from the base image, so repeat the `remoteUser` and `containerUser` it declares. `scripts/check-image-path.sh` asserts this in `build-checks.yml`, where `scripts/devcontainer-path.sh` resolves the label the way a client does, so the smoke test runs against the `PATH` a client applies.
