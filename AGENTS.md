...
- Every image runs as the non-root user `dev` and ships nothing `dev` owns outside `/home/dev`, which `build-checks.yml` asserts per image with `scripts/check-image-ownership.sh`. Two things depend on it: a Dev Container client remaps `dev` to the host user's UID and GID by default on Linux and re-owns the home directory alone, so anything else `dev` owns is left under a UID no account holds; and root ownership is what keeps the toolchain out of reach of the code the container runs. When writing a Dockerfile:
  - Extract an upstream archive with `tar --no-same-owner`. tar running as root restores the uid the archive records, and some upstreams build theirs as uid 1000.
  - Own a directory `dev` has to write to outside its home as `root:<group>` with `chmod 2775`, adding `dev` to the group. Group membership survives the remap because `/etc/group` names the member; user ownership does not.
  - Create a directory before the `USER` instruction that switches to `dev`, since anything created after it is owned by `dev`. `/workspaces` is created that way in the base image.
